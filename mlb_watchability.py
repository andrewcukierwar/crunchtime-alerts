...
def get_team_ratings():
    ...
    content = soup.find(class_='index-standings-table').find('tbody').find_all('tr')

    ratings = {}
    for tr in content:
        rating = int(tr.find('td', class_='num rating').text)
        team = tr.find('td', class_='team').find('a').text
        ratings[team] = rating
    return ratings
